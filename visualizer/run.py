...
"""Run training/testing of one or more algorithmic tasks from CLRS.

Visualizer attachment:
- In test mode, exports all probes + (optional) GNN hidden states to JSON under
  `visualizer/extracted/`.

This file is intentionally kept outside of the `clrs/` package so that the
visualizer can be used as an add-on without modifying upstream CLRS sources.
"""
...
# Import visualizer helpers (kept outside core clrs package).
# When invoked as `python -m visualizer.run`, prefer a relative import.
try:
...
except ImportError:  # Fallback for `python visualizer/run.py`
    from probe_utils import (
        extract_all_probes,
        quantize_floats_in_obj,
        save_algo_to_json,
...
flags.DEFINE_integer(
    'length_needle',
    -8,
    'Length of needle for training and validation (not testing) in string '
    'matching algorithms. A negative value randomizes the length for each '
    'sample between 1 and the opposite of the value. A value of 0 means use '
    'always 1/4 of the length of the haystack (the default sampler behavior).',
)
...
flags.DEFINE_integer('val_batch_size', 32, 'Batch size used for validation.')
flags.DEFINE_integer('test_batch_size', 32, 'Batch size used for testing.')
flags.DEFINE_boolean('chunked_training', False, 'Whether to use chunking for training.')
flags.DEFINE_integer('chunk_length', 16, 'Time chunk length used for training (if chunked_training is True).')
...
flags.DEFINE_string(
    'val_log_path',
    '',
    'Optional CSV file to append validation metrics every eval (columns: step,algorithm,score).',
)

flags.DEFINE_integer('hidden_size', 128, 'Number of hidden units of the model.')
flags.DEFINE_integer('nb_heads', 1, 'Number of heads for GAT processors')
flags.DEFINE_integer('nb_msg_passing_steps', 1, 'Number of message passing steps to run per hint.')
...
flags.DEFINE_boolean('use_ln', True, 'Whether to use layer normalisation in the processor.')
...
flags.DEFINE_string('checkpoint_path', './checkpoints', 'Path in which checkpoints are saved.')
flags.DEFINE_string('dataset_path', '/tmp/CLRS30', 'Path in which dataset is stored.')
...
def make_sampler(
  ...
  if length < 0:  # load from file
    ...
    sampler, num_samples, spec = clrs.create_dataset(
        folder=dataset_folder, algorithm=algorithm, batch_size=batch_size, split=split
    )
...
def main(unused_argv):
  ...
  if FLAGS.mode == 'train':
    ...
    while step < FLAGS.train_steps:
      ...
      if step == 0:
        ...
        if FLAGS.chunked_training:
          all_length_features = [all_features] + [
              [next(t).features for t in train_samplers] for _ in range(len(train_lengths))
          ]
      ...
      for algo_idx in range(len(train_samplers)):
        ...
        logging.info(
            'Algo %s step %i current loss %f, current_train_items %i.',
            FLAGS.algorithms[algo_idx],
            step,
            cur_loss,
      ...
      if step >= next_eval:
        ...
        for algo_idx in range(len(train_samplers)):
          ...
          if FLAGS.val_log_path:
            try:
              write_header = (
                  not os.path.exists(FLAGS.val_log_path)
                  or os.path.getsize(FLAGS.val_log_path) == 0
              )
              with open(FLAGS.val_log_path, 'a') as f:
                if write_header:
                  f.write('step,algorithm,score\n')
                f.write(f"{step},{FLAGS.algorithms[algo_idx]},{val_stats.get('score', float('nan'))}\n")
            except Exception as e:
              logging.warning('Failed to write val log to %s: %s', FLAGS.val_log_path, e)

        next_eval += FLAGS.eval_every

        msg = (
            f'best avg val score was {best_score/len(FLAGS.algorithms):.3f}, '
            f'current avg val score is {np.mean(val_scores):.3f}, val scores are: '
        )
        msg += ', '.join([
  ...
  if FLAGS.mode == 'test':
    ...
    init_features = [next(t).features for t in test_samplers]
    eval_model.init(init_features, FLAGS.seed + 1)
    eval_model.restore_model('best.pkl', only_load_processor=False)

    # Export directory relative to this file:
    ...
    for algo_idx in range(len(train_samplers)):
      ...
      # When return_all_outputs=True, predict returns the output predictions as well.
      # We capture them so the JSON export can include real OUTPUT preds.
      out_preds, hint_preds, hidden_states = eval_model.predict(
           new_rng_key,
      ...
      # Always export hidden states for PCA.
      # The viewer enables PCA toggles only if algo.gnn_hidden_states.data exists.
      algo_data = extract_all_probes(
           hint_preds,
           test_sample,
           include_gnn_hidden_states=True,
           output_preds=out_preds,
           gnn_hidden_states=hidden_states,
       )
      ...
      # Keep original evaluation behavior.
      common_extras = {'examples_seen': 0, 'step': step, 'algorithm': algo_name}
      ...
      logging.info('(test) algo %s : %s', algo_name, test_stats)
